...
def calculate_cost(
    ...
    if prices is None:
        if model not in _warned_unknown:
            ...
            logger.warning(
                "llm-cost: unknown model %r, pricing as %s (the most "
...
def staleness_warning(today: date | None = None, max_age_days: int = 120) -> str | None:
    ...
    if age <= max_age_days:
        return None
    return (
        f"llm-cost price table is {age} days old (reviewed "
...
def _env_float(name: str, default: float | None = None) -> float | None:
    ...
    try:
        ...
        logger.warning("llm-cost: %s=%r is not a number, ignoring", name, raw)
        return default
