...
from llm_cost.cli import main as cli_main

SRC = Path(llm_cost.__file__).parent
...
def test_core_has_no_required_dependencies():
    ...
    for py in sorted(SRC.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for n in names:
                root = n.split(".")[0]
                if root in sys.stdlib_module_names or root == "llm_cost":
                    continue
...
def test_importing_needs_no_api_key(monkeypatch):
    ...
    from llm_cost import TrackedClient
