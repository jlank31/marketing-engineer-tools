...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--version", action="version", version=f"editor-rules {__version__}")
    args = ap.parse_args(argv)

    try:
        edits = load(args.path)
    except (OSError, ValueError) as e:
        print(f"editor-rules: {e}", file=sys.stderr)
        return 2

    if not edits:
        print(
            f"editor-rules: no usable pairs in {args.path}.\n"
    ...
    if args.json:
        ...
        if not text:
            print(
                "editor-rules: nothing survived the prompt filter.\n"
