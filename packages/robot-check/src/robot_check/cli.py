"""robot-check command line interface.

    robot-check scan draft.md
    robot-check scan posts/*.md --preset social
    cat draft.md | robot-check scan -
    robot-check scan article.md --preset blog --siblings ./published --json
    robot-check profile --preset blog > profile.json
...
def _scan(args) -> int:
    ...
    for target in args.paths:
        try:
            ...
            print(f"robot-check: cannot read {target}: {e}", file=sys.stderr)
            worst = max(worst, 2)
            continue
...
def _corpus_cmd(args) -> int:
    ...
    root = Path(args.dir)
    if not root.is_dir():
        print(f"robot-check: not a directory: {root}", file=sys.stderr)
        return 2

    corpus = load_corpus(root, pattern=args.pattern,
                         exclude_section=args.exclude_section)
    if not corpus:
        print(f"robot-check: no files matching {args.pattern!r} under {root}",
              file=sys.stderr)
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--version", action="version", version=f"robot-check {__version__}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"robot-check: {e}", file=sys.stderr)
        return 2
