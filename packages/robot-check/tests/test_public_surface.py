...
import pytest
import robot_check
from robot_check import Profile, check_text
from robot_check.cli import main as cli_main
from robot_check.profiles import PRESETS, from_dict, to_dict

SRC = Path(robot_check.__file__).parent
...
def test_imports_with_no_environment():
    ...
    assert robot_check.__version__
    assert callable(check_text)


def test_advertised_names_are_exported():
    for name in robot_check.__all__:
        assert hasattr(robot_check, name), f"__all__ promises {name} but it is missing"
...
def test_package_has_zero_third_party_imports():
    ...
    for py in sorted(SRC.rglob("*.py")):
        ...
        for node in ast.walk(tree):
            ...
            for n in names:
                root = n.split(".")[0]
                if root not in sys.stdlib_module_names and root != "robot_check":
...
def test_cli_reads_stdin():
    """`cat draft.md | robot-check scan -` is the documented pipe form."""
    proc = subprocess.run(
        [sys.executable, "-m", "robot_check.cli", "scan", "-"],
...
def test_corpus_finds_verbatim_reuse_and_never_fails(tmp_path):
    ...
    from robot_check.corpus import analyze, load_corpus
