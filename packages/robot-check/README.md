...
```bash
robot-check scan draft.md                          # one file
robot-check scan posts/*.md --preset social        # tighter limits for short copy
cat draft.md | robot-check scan -                  # stdin
robot-check scan article.md --preset blog --siblings ./published
robot-check scan drafts/*.md --json                # for CI
robot-check profile --preset blog > profile.json   # then edit and pass --profile
```
