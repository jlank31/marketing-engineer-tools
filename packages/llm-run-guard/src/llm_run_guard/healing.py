        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(attempts):
                ...
                try:
                    ...
                    if is_fatal(e):
                        _bump("failures")
                        logger.error("llm-run-guard: fatal, not retrying: %s", e)
                        raise
                    ...
                    _bump("retries")
                    logger.warning(
                        "llm-run-guard: %s failed (attempt %d/%d), retrying in %.1fs: %s",
            ...
            if diagnose is not None:
                ...
                try:
                    ...
                    if note:
                        logger.error("llm-run-guard: diagnosis: %s", note)
                except Exception:  # noqa: BLE001
                    logger.debug("llm-run-guard: diagnosis hook failed", exc_info=True)
