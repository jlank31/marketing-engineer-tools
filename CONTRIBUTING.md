...
Three files are maintained in a private repository and copied here:

- `packages/robot-check/src/robot_check/content_quality.py`
- `packages/robot-check/src/robot_check/repetition.py`
- `packages/robot-check/src/robot_check/post_validators.py`

`packages/robot-check/VENDORED.sha256` records their hashes, and CI fails if any of them is edited in place. `make mirror` checks the same thing locally.
...
**Actively wanted:** price-table corrections in `llm-cost`. A published price table goes stale within months, and a cost tool reporting stale numbers is worse than no cost tool. These are trivial to review and keep the package honest.
...
**Usually declined:** new detectors and new features. Not because they're bad ideas. Every rule here earns its place by having caught something real in production, and I can't vouch for one that hasn't. Fork freely; that's what the license is for.

## If you send a detector change

Include two tests: one for the thing it now catches, and one for something that must keep passing. The second matters more. A regex that catches more is easy; a regex that catches more *without* flagging ordinary prose is the hard part.
...
This repo has a stated scope: 8 drops across Aug to Sep 2026, then maintenance mode. Maintenance means bugs and price updates, not new surface area. Saying so up front seems better than going quiet in October and letting you wonder.
